...
Color PointLight::getContribution(const Scene& scene, const Ray& ray, HitInfo& hit) {
    ...
    if (!(b_hit && shadowHit.distance < light_distance)) {
        ...
        final_color = final_color + hit.material->diffuse * attenuated_color * std::max(0.0f, dot(hit.normal.normalized(), direction.normalized()));
        Direction3 h = (direction - ray.dir).normalized();
        final_color = final_color + hit.material->specular * attenuated_color * pow(std::max(0.0f, dot(hit.normal.normalized(), h)), hit.material->ns);
    }
    return final_color;
}
